...
<body>
    ...
    {{-- ══════════════════════════════════════════════════════════════════════
         SECCIÓN EXCLUSIVA PARA TUTORES — alumnos relacionados + pagos
    ══════════════════════════════════════════════════════════════════════ --}}
    ...
    <div class="main-content" style="margin-top:0">
      <div class="content p-4">
        <div class="info-section" style="border-top:4px solid #e53935; margin-top:0">
            <div class="section-header">
                <div class="section-icon" style="background:linear-gradient(135deg,#ffebee,#ffcdd2)">
                    <i class="bi bi-people-fill" style="color:#e53935;font-size:1.1rem"></i>
                </div>
                <h2 class="section-title">Mis Alumnos Relacionados</h2>
            </div>

            @php
                $alumnosRelacionados = \Illuminate\Support\Facades\DB::table('tutor_alumno as ta')
                    ->join('usuario as a', 'ta.id_alumno', '=', 'a.id_usuario')
                    ->where('ta.id_tutor', auth()->id())
                    ->select(
                        'ta.id_alumno',
                        'ta.relacion',
                        \Illuminate\Support\Facades\DB::raw("CONCAT(a.nombre,' ',a.apaterno,' ',a.amaterno) AS nombre_alumno"),
                        \Illuminate\Support\Facades\DB::raw("a.nombre AS primer_nombre"),
                        \Illuminate\Support\Facades\DB::raw("a.apaterno AS primer_apellido")
                    )
                    ->get();
            @endphp
    ...
    function submitNuevoPago(e, idAlumno) {
        ...
        .then(r => {
            // PagoController@store redirige (302) o devuelve JSON según Accept header
