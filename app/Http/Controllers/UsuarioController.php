...
class UsuarioController extends Controller
{
    ...
    // Solo cambiar este valor si algún día se cambia el correo del superusuario.

    private const SUPERUSUARIO_CORREO = 'nailea8040@gmail.com';

    /**
     * Retorna true si el usuario dado (objeto o id) es el superusuario.
     * Acepta un objeto $usuario (con ->correo) o null.
     */
    ...
    /**
     * Retorna true si el usuario autenticado actualmente es el superusuario.
     */
    private function authEsSuperUsuario(): bool
    {
        return Auth::check() && $this->esSuperUsuario(Auth::user());
    }
    ...
    public function edit(int $id)
    {
        ...
        // Solo el propio superusuario puede acceder a su formulario de edición
        if ($this->esSuperUsuario($usuario) && !$this->authEsSuperUsuario()) {
            return redirect()->route('usuarios.index')
                ->with('sessionInsertado', 'false')
                ->with('mensaje', 'No tienes permisos para editar al superusuario del sistema.');
        }
    ...
    public function update(Request $request, int $id)
    {
        ...
        if (!$usuarioActual) {
            ...
            if ($request->expectsJson()) {
                return response()->json(['ok' => false, 'mensaje' => $msg], 404);
            }
            return redirect()->route('usuarios.index')
                ->with('sessionInsertado', 'false')
                ->with('mensaje', $msg);
        ...
        // ── Protección superusuario ──────────────────────────────────────────
        // Solo el propio superusuario puede editar su cuenta
        if ($this->esSuperUsuario($usuarioActual) && !$this->authEsSuperUsuario()) {
            $msg = 'No tienes permisos para editar al superusuario del sistema.';
            if ($request->expectsJson()) {
                return response()->json(['ok' => false, 'mensaje' => $msg], 403);
            }
            return redirect()->route('usuarios.index')
        ...
        // ── Protección de rol del superusuario ───────────────────────────────
        // Aunque sea el propio superusuario quien edita, el rol nunca cambia
        if ($this->esSuperUsuario($usuarioActual)) {
            $validated['rol'] = $usuarioActual->rol; // forzar el rol original
        }
    ...
    public function destroy(int $id)
    {
        ...
        try {
            ...
            if ($this->esSuperUsuario($usuario)) {
                return redirect()->route('usuarios.index')
                    ->with('sessionInsertado', 'false')
                    ->with('mensaje', 'El superusuario del sistema no puede ser eliminado.');
            }
    ...
    public function toggleActive(int $id)
    {
        ...
        try {
            ...
            if ($this->esSuperUsuario($usuario)) {
                return redirect()->route('usuarios.index')
                    ->with('sessionInsertado', 'false')
                    ->with('mensaje', 'El superusuario del sistema no puede ser desactivado.');
            }
